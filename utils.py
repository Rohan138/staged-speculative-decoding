...
def get_mismatches(og_outputs, new_outputs, dtype=None):
    ...
    for i in range(num_samples):
        if og_outputs[i] != new_outputs[i]:
            ...
            if (
                dtype is None
            ):  # float 16 is a bit unstable, float 32 gets the same results
                print("\nOG :", og_outputs[i])
                print("NEW:", new_outputs[i])
    print(f"Mismatches: {mismatches}")
    if dtype is not None:
        print(
            "Note: dtype is NOT float32, so mismatches can happen due to numerical instability"
        )
...
def run_og_model(args, processor_cls, model_cls, run_prediction_loop, queue):
    ...
    og_outputs = run_prediction_loop(
...
def run_new_model(args, processor_cls, model_cls, run_prediction_loop, queue):
    ...
    new_outputs = run_prediction_loop(
        model, tokenizer, args.num_samples, args.temperature, aux_model
    )

...
def staged_speculative_decoding(
    ...
    while True:
        ...
        tree = DecodingTree(draft_inputs, draft_model, temperature, num_tokens, depth)
        ...
        for idx in range(len(tree_input_ids)):
            pad_length = depth + 1 - tree_input_ids[idx].shape[1]
            if pad_length > 0:
        ...
        attention_mask = torch.cat(
            [mask, mask.new_ones((mask.shape[0], depth + 1))], dim=-1
        )
